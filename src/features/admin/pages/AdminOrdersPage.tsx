...
const StatBlock = ({
  ...
  >
    ...
    <div className="relative">
      <div className={`text-[10px] font-black uppercase tracking-[0.28em] ${dark ? "text-slate-400" : "text-slate-400"}`}>
        {title}
      </div>
...
export default function AdminOrdersPage() {
  ...
  const orderMapContainerId = "admin-order-detail-google-map";

  if (loading) return <div>Loading admin orders...</div>;
    ...
    <div className="space-y-8 pb-20">
      <div className="grid gap-6 md:grid-cols-2">
        <StatBlock
          ...
          hint="Seluruh order yang masuk ke monitoring admin."
        />
        <StatBlock
          title="Attention Queue"
          value={String(attentionOrders.length)}
          hint="Order yang perlu reaksi cepat operator."
        />
      </div>
            ...
            <div className="flex w-full flex-col gap-4 xl:w-auto xl:flex-row xl:items-center">
              <div className="relative w-full xl:w-96">
                ...
                <input
                  ...
                  placeholder="Cari transaksi ID, customer, merchant, driver..."
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-3">
                    <h2 className="truncate text-2xl font-black text-slate-900">
                      Order #{String(selectedOrder.id || "-").slice(0, 10)}
                    </h2>
                  ...
                  <p className="mt-2 text-sm text-slate-500">
                    {selectedOrder.restaurantName || selectedOrder.restaurantId || "-"} → {selectedOrder.customerName || selectedOrder.customerId || "-"}
                  </p>
                </div>
                ...
                <div className="mt-6 space-y-6">
                  <div className="grid gap-4 grid-cols-3">
                    <div className="rounded-2xl bg-sky-50 p-4">
                      ...
                      <div className="mt-2 text-xl font-black text-slate-900">{formatMoney(selectedOrder.total ?? selectedOrder.totalPrice)}</div>
                    ...
                    <div className="rounded-[28px] border border-slate-200 bg-white p-5">
                      ...
                      <div className="mt-4 space-y-3">
                        {[
                          ...
                          { label: "Resto Earnings", value: formatMoney(selectedOrder.restoEarnings) },
                    ...
                    <div className="mt-4 grid gap-3 grid-cols-2">
                      ...
                      <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm col-span-2">
                        ...
                        <span className="text-slate-600">
                          {selectedOrder.cancelledAt || selectedOrder.cancelledBy || selectedOrder.cancellationReason
                            ? `${formatTime(selectedOrder.cancelledAt)} | ${selectedOrder.cancelledBy || "SYSTEM"} | ${selectedOrder.cancellationReason || "-"}`
                            : "-"}
                        </span>
                      </div>
                      <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm col-span-2">
                        <span className="font-semibold text-slate-900">Updated:</span>{" "}
                        <span className="text-slate-600">{formatTime(selectedOrder.updatedAt)}</span>
                      </div>
                    </div>
                  </div>
                </div>
